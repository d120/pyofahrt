...
class Slot(models.Model):
    ...
    SLOT_CHOICES = (("workshop", "Workshop"), ("event",
                                               "Zentrale Veranstaltung"),
                    ("food", "Essen"), ("sonstiges", "Sonstiges"))

    PRIORITY_CHOICES = (("opt", "optional"), ("zentral", "zentral"),
                        ("dezentral", "dezentral"), ("wichtig", "wichtig"))

    slottype = models.CharField(
        "Art des Slots",
...
class Workshop(models.Model):
    class Meta:
        ...
        permissions = (("can_use", "Die Workshopkomponente nutzen"),
    ...
    host = models.ManyToManyField(
        User, verbose_name="Workshopanbieter", blank=True)

    maxmembers = models.IntegerField(
        "Maximale Teilnehmerzahl", blank=True, default=0)
    otherstuff = models.TextField("Sonstiges", blank=True)

    accepted = models.BooleanField(
        "Akzeptiert", help_text="Workshopidee ist in Ordnung", default=False)
    ...
    slot = models.ForeignKey(
        Slot, verbose_name="Zugeteilter Zeitslot", null=True, blank=True)
